...
static int smt_send(fwk_id_t channel_id, struct mod_smt_command_config *cmd)
{
    ...
    memory->message_header =
        SCMI_MESSAGE_HEADER(cmd->message_id, cmd->protocol_id, 0);
    ...
    const void *data)
{
    smt_ctx.channel_ctx_table =
        fwk_mm_calloc(element_count, sizeof(smt_ctx.channel_ctx_table[0]));
    ...
    const void *data)
{
    ...
    channel_ctx->max_payload_size =
        channel_ctx->config->mailbox_size - sizeof(struct mod_smt_memory);
...
static int smt_process_bind_request(
    fwk_id_t source_id,
    fwk_id_t target_id,
    fwk_id_t api_id,
    const void **api)
{
    ...
    channel_ctx = &smt_ctx.channel_ctx_table[fwk_id_get_element_idx(target_id)];
