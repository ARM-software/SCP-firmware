...
static float dmc_clk_period_ps;

struct ddr4_spd ddr4_dimm0;
struct ddr4_spd ddr4_dimm1;
...
static int chk_ddr4_dimms(
    unsigned int speed,
    struct ddr4_spd *dimm0,
    struct ddr4_spd *dimm1)
{
...
static void dimm_device_data(const uint8_t *spd_data, uint8_t dimm_id)
{
    ...
    if (spd_data[2] == 0x0C) {
        ...
        FWK_LOG_ERR("[DDR] ERROR! DDR4 SPD EEPROM Not Detected");
        fwk_unexpected();
    }
}

static int get_dimm_rank_bits(
    uint8_t element,
    uint32_t *temp_reg,
    struct dimm_info *ddr)
{
    int status;
    uint8_t pkg_rank =
        (element & SPD_PKG_RANK_BITS_MASK) >> SPD_PKG_RANK_BITS_OFFSET;
...
static int get_dimm_memory_device_width_next_bits(
    uint8_t element,
    uint32_t *temp_reg,
    struct dimm_info *ddr)
{
...
int dimm_spd_address_control(uint32_t *temp_reg, struct dimm_info *ddr)
{
    ...
    status = get_dimm_col_bits(ddr4_dimm0.dram_param.sdram_addr, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_row_bits(ddr4_dimm0.dram_param.sdram_addr, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_bank_addr_grp_bits(
        ddr4_dimm0.dram_param.sdram_density_banks, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_rank_bits(ddr4_dimm0.dram_param.mod_org, temp_reg, ddr);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_cid_bits(ddr4_dimm0.dram_param.sdram_pkg_type, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }
...
int dimm_spd_format_control(uint32_t *temp_reg)
{
    ...
    status = get_dimm_memory_width(
        ddr4_dimm0.dram_param.mod_mem_bus_width, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }
...
int dimm_spd_memory_type(uint32_t *temp_reg, struct dimm_info *ddr)
{
    ...
    status =
        get_dimm_memory_type_nxt(ddr4_dimm0.dram_param.kb_dram_type, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_row_bits(ddr4_dimm0.dram_param.kb_dram_type, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_bank_grp_bits(
        ddr4_dimm0.dram_param.sdram_density_banks, temp_reg);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = get_dimm_memory_device_width_next_bits(
        ddr4_dimm0.dram_param.mod_org, temp_reg, ddr);
    if (status != FWK_SUCCESS) {
        return status;
    }
...
void dimm_spd_t_refi(uint32_t *temp_reg)
{
    ...
    *temp_reg = (T_REFI_NEXT_MASK & tmp_refi_now);
    ...
    fwk_assert(*temp_reg != 0);
}

void dimm_spd_t_rfc(uint32_t *temp_reg)
{
    ...
    *temp_reg &= ~T_RFCFC_NEXT_MASK;
    ...
    if (multi_rank == true) {
        dimm_spd_t_refi(&tmp_value);
        *temp_reg |=
            (T_RFCFC_NEXT_MASK & ((uint32_t)((float)tmp_value / t_refi) << 20));
    }

    fwk_assert(*temp_reg != 0);
}

void dimm_spd_t_rcd(uint32_t *temp_reg)
{
    uint8_t spd_tmp = 0;
    uint8_t spd_tmp2 = 0;
    uint32_t tmp_value = 0;
...
void dimm_spd_t_ras(uint32_t *temp_reg)
{
    uint8_t temp = 0;
    uint32_t tras_tmp = 0;
    uint32_t tmp_value = 0;
...
void dimm_spd_t_rp(uint32_t *temp_reg)
{
    uint8_t spd_tmp = 0;
    uint8_t spd_tmp2 = 0;
    uint32_t tmp_value = 0;
...
void dimm_spd_t_rrd(uint32_t *temp_reg)
{
    uint8_t spd_tmp = 0;
    uint8_t spd_tmp2 = 0;
    uint32_t tmp_value = 0;
...
void dimm_spd_t_wtr(uint32_t *temp_reg, struct dimm_info *ddr)
{
    uint8_t spd_tmp1 = 0;
    uint8_t spd_temp = 0;
    uint32_t twtr_tmp = 0;
