...
#define NUM_DQ_BITS            (NUM_SLICES) * (NUM_BITS_PER_SLICE)
#define NUM_DFI_BEATS_TO_CHECK UINT32_C(4)

#define SLICE_MASK (~(UINT32_C(0xFFFFFFFF) << (NUM_BITS_PER_SLICE)))

#define NUM_WORDS_IN_DFI_BEAT UINT32_C(5)
...
uint16_t DEFAULT_DELAY_V2 = 0x240;
...
int morello_ddr_phy_config(fwk_id_t element_id, struct dimm_info *info)
{
    ...
    ddr_phy = (struct mod_morello_ddr_phy_reg *)element_config->ddr_phy_base;
    ...
    uint32_t bank)
{
    ...
    if ((dmc->MEMC_STATUS & 0x7) != 0x0) {
        FWK_LOG_ERR("DMC needs to be in config state");
        return FWK_E_STATE;
    }
    ...
    while ((dmc->MEMC_STATUS & MOD_DMC_BING_MEMC_STATUS_MGR_ACTIVE) != 0) {
        ;
    }
...
int write_eye_detect_single_rank(
    fwk_id_t element_id,
    struct dimm_info *info,
    uint32_t rank,
    uint32_t delay_increment,
    uint32_t vrefdq_increment,
    uint32_t dbg_level)
{
    ...
    int s;
    int t;
    struct mod_dmc_bing_reg *dmc = NULL;
    uint32_t ddr_phy_base = 0;

    fwk_assert(rank < 2);
    dmc_id = fwk_id_get_element_idx(element_id);
    if (dmc_id == 0) {
        dmc = (struct mod_dmc_bing_reg *)SCP_DMC0;
        ddr_phy_base = SCP_DDR_PHY0;
    } else if (dmc_id == 1) {
        dmc = (struct mod_dmc_bing_reg *)SCP_DMC1;
    ...
    for (vrefdq_mr6 = MIN_VREFDQ_MR6; vrefdq_mr6 <= MAX_VREFDQ_MR6;
            ...
            while (num_completed != NUM_DQ_BITS) {
                ...
                for (slice = 0; slice < NUM_SLICES; slice++) {
                    for (bit = 0; bit < NUM_BITS_PER_SLICE; bit++) {
                        ...
                        if (bit % 2 == 1) {
                            reg_val = (cur_wrdq_delays[slice][bit] & 0x7FF)
                                << 16;
                            reg_val |=
                ...
                *(uint32_t
                      *)(ddr_phy_base + (4 * SC_PHY_MANUAL_UPDATE_REG_IDX)) =
                    sc_phy_manual_update_reg_val;

                (void)memset(wrrd_passes, 0, sizeof(uint8_t) * NUM_DQ_BITS);

                for (data_pattern = 0; data_pattern < NUM_DATA_PATTERNS;
                     data_pattern++) {
                    wr_data = wr_data_all[data_pattern];
                    ret_code =
                        dci_write_dram(dmc, wr_data, DCI_FIFO_SIZE, rank, 0);
                    if (ret_code != FWK_SUCCESS) {
                        return ret_code;
                    }
                    ret_code =
                        dci_read_dram(dmc, rd_data, DCI_FIFO_SIZE, rank, 0);
                    if (ret_code != FWK_SUCCESS) {
                        return ret_code;
                    }

                    for (dfi_beat = 0; dfi_beat < NUM_DFI_BEATS_TO_CHECK;
                         dfi_beat++) {
                        ...
                        for (dqs_edge = 0; dqs_edge < 2; dqs_edge++) {
                            ...
                            for (slice = 0; slice < NUM_SLICES; slice++) {
                                ...
                                word_num =
                                    (start_bit / 32) + dfi_beat_word_offset;
                                wr_slice_data =
    ...
    uint32_t dbg_level)
{
    ...
    for (rank = start_rank; rank <= stop_rank; rank++) {
        status = write_eye_detect_single_rank(
            element_id,
            info,
            rank,
            delay_increment,
    ...
    struct dimm_info *info)
{
    ...
    element_config = fwk_module_get_data(element_id);
    phy_addr = (uint32_t)element_config->ddr_phy_base;
    ...
    struct dimm_info *info)
{
    ...
    element_config = fwk_module_get_data(element_id);
    phy_base = (uint32_t)element_config->ddr_phy_base;
    num_slices = 18;
...
int morello_read_eye_phy_obs_regs(fwk_id_t element_id, struct dimm_info *info)
{
    const struct mod_dmc_bing_element_config *element_config;
    uint32_t i;
    uint32_t j;
    ...
    struct dimm_info *info)
{
    ...
    status = morello_wrlvl_phy_obs_regs(element_id, rank, info);
    if (status != FWK_SUCCESS) {
        return status;
    }

    status = morello_read_gate_phy_obs_regs(element_id, rank, info);
    if (status != FWK_SUCCESS) {
        return status;
    }

    return morello_read_eye_phy_obs_regs(element_id, info);
}
