...
static const uint32_t feature_flags =
    (SGI575_SDS_FEATURE_FIRMWARE_MASK | SGI575_SDS_FEATURE_DMC_MASK |
...
static int messaging_stack_ready(void)
{
     ...
     * stack
     */
    return sgi575_system_ctx.sds_api->struct_write(
...
static void ppu_clusters_isr(void)
{
    ...
    while (status != 0) {
        ...
        sgi575_system_ctx.ppu_v1_isr_api->ppu_interrupt_handler(FWK_ID_ELEMENT(
            FWK_MODULE_IDX_PPU_V1, sgi575_core_get_core_count() + cluster_idx));
...
static int sgi575_system_shutdown(enum mod_pd_system_shutdown system_shutdown)
{
    NVIC_SystemReset();
...
static int sgi575_system_mod_init(
    fwk_id_t module_id,
    unsigned int unused,
    const void *unused2)
{
...
static int sgi575_system_process_bind_request(
...
int sgi575_system_process_notification(
