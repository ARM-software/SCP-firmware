...
static_assert(
    FWK_ARRAY_SIZE(sds_module_regions) == SGI575_SDS_REGION_COUNT,
...
static_assert(
    SCP_SDS_SECURE_SIZE > SGI575_SDS_CPU_INFO_SIZE +
            SGI575_SDS_FIRMWARE_VERSION_SIZE + SGI575_SDS_PLATFORM_ID_SIZE +
...
static_assert(
    SCP_SDS_NONSECURE_SIZE >
        SGI575_SDS_CPU_BOOTCTR_SIZE + SGI575_SDS_CPU_FLAGS_SIZE,
...
static const struct fwk_element *sds_get_element_table(fwk_id_t module_id)
{
    ...
    platid.platform_type_identifier =
        platid.platform_identifier & SGI575_SDS_PLATID_PARTNO_MASK;
