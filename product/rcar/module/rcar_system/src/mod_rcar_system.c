...
{
    memcpy((void *)SCP_SRAM_BASE, (void *)SRAM_COPY_START,
...
static const struct mod_system_power_driver_api
    rcar_system_system_power_driver_api = {
    .system_shutdown = rcar_system_shutdown
};
...
static int rcar_system_init(fwk_id_t module_id, unsigned int element_count,
                            const void *data)
{
    ...
    module_ctx.dev_ctx_table = fwk_mm_calloc(element_count,
                                             sizeof(struct rcar_system_dev_ctx));
    if (module_ctx.dev_ctx_table == NULL)
        return FWK_E_NOMEM;

    return FWK_SUCCESS;
}
static int rcar_system_element(fwk_id_t element_id, unsigned int sub_element_count,
                            const void *data)
{
    struct rcar_system_dev_ctx *ctx;
...
static int rcar_system_process_bind_request(fwk_id_t source_id,
...
static int rcar_system_process_notification(const struct fwk_event *event,
