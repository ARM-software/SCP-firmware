 ...
 * Framework handlers
 */
static int mfismh_init(fwk_id_t module_id, unsigned int device_count,
                    const void *unused)
{
    if (device_count == 0)
        return FWK_E_PARAM;

    mfismh_ctx.device_ctx_table = fwk_mm_calloc(device_count,
        sizeof(mfismh_ctx.device_ctx_table[0]));
...
static int mfismh_device_init(fwk_id_t device_id, unsigned int slot_count,
                           const void *data)
{
...
static int mfismh_process_bind_request(fwk_id_t source_id, fwk_id_t target_id,
