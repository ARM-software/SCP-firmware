 ...
 */
static int get_rate_entry(struct rcar_sd_clock_dev_ctx *ctx,
 ...
 */
static int sd_clock_set_rate(fwk_id_t dev_id, uint64_t rate,
...
static int sd_clock_get_rate_from_index(fwk_id_t dev_id,
...
{
    fwk_id_t element_id = FWK_ID_ELEMENT_INIT(FWK_MODULE_IDX_CLOCK, CLK_ID_SD_START);
    struct rcar_sd_clock_dev_ctx *ctx;
    uint32_t sd_id = 0;
    int ret = FWK_SUCCESS;
...
static int sd_clock_init(fwk_id_t module_id, unsigned int element_count,
                          const void *data)
{
    ...
    module_ctx.dev_ctx_table = fwk_mm_calloc(element_count,
                                         sizeof(struct rcar_sd_clock_dev_ctx));
...
static int sd_clock_element_init(fwk_id_t element_id,
                                  ...
                                  const void *data)
{
    ...
    if (ctx->config->rate_count > 0) {
        ctx->rate_table = fwk_mm_calloc(ctx->config->rate_count,
                                             sizeof(uint64_t));
...
static int sd_clock_process_bind_request(fwk_id_t source_id,
