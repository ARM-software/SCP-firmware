...
static void mstp_clock_hw_initial_set_state(fwk_id_t element_id,
                                        struct rcar_mstp_clock_dev_ctx *ctx)
{
...
{
    fwk_id_t element_id = FWK_ID_ELEMENT_INIT(FWK_MODULE_IDX_CLOCK,
                                              CLK_ID_MSTP_START);
    uint32_t mstp_id;
    struct rcar_mstp_clock_dev_ctx *ctx;
...
static int mstp_clock_set_rate(fwk_id_t dev_id, uint64_t rate,
...
static int mstp_clock_get_rate_from_index(fwk_id_t dev_id,
...
static int mstp_clock_init(fwk_id_t module_id, unsigned int element_count,
                          const void *data)
{
    ...
    module_ctx.dev_ctx_table = fwk_mm_calloc(element_count,
                                     sizeof(struct rcar_mstp_clock_dev_ctx));
...
static int mstp_clock_element_init(fwk_id_t element_id,
...
static int mstp_clock_process_bind_request(fwk_id_t source_id,
