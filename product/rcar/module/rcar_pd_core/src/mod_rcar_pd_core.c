...
static int rcar_cluster_pd_set_state(fwk_id_t cluster_pd_id,
                                       unsigned int state)
{
    return FWK_SUCCESS;
}
...
static int rcar_mod_init(fwk_id_t module_id, unsigned int pd_count,
                           const void *unused)
{
    rcar_pd_sysc_ctx.pd_ctx_table = fwk_mm_calloc(pd_count,
                                            sizeof(struct rcar_pd_sysc_pd_ctx));
...
static int rcar_core_process_bind_request(fwk_id_t source_id,
