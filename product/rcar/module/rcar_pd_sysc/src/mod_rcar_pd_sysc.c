...
static int rcar_sysc_mod_init(fwk_id_t module_id, unsigned int pd_count,
                           const void *unused)
{
    rcar_sysc_ctx.pd_ctx_table = fwk_mm_calloc(pd_count,
                                            sizeof(struct rcar_sysc_pd_ctx));
...
static int rcar_sysc_pd_init(
    fwk_id_t pd_id, unsigned int unused, const void *data)
{
...
static int rcar_sysc_process_bind_request(fwk_id_t source_id,
