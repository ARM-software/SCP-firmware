...
static unsigned int scmi_notification_table[1] = {
    (unsigned int)SCP_TC1_SCMI_SERVICE_IDX_PSCI,
};

/* Module context */
struct tc1_system_ctx {
...
struct tc1_system_isr {
    unsigned int interrupt;
    void (*handler)(void);
};

static struct tc1_system_ctx mod_ctx;

static const uint32_t feature_flags =
    (TC1_SDS_FEATURE_FIRMWARE_MASK | TC1_SDS_FEATURE_DMC_MASK |
     TC1_SDS_FEATURE_MESSAGING_MASK);
...
static int tc1_system_shutdown(enum mod_pd_system_shutdown system_shutdown)
{
    NVIC_SystemReset();
...
static const struct mod_system_power_driver_api
    tc1_system_system_power_driver_api = {
        .system_shutdown = tc1_system_shutdown,
    };
...
static int tc1_system_mod_init(
    fwk_id_t module_id,
    unsigned int unused,
    const void *unused2)
{
...
static int tc1_system_bind(fwk_id_t id, unsigned int round)
{
    int status;
...
static int tc1_system_process_bind_request(
    fwk_id_t requester_id,
    fwk_id_t pd_id,
    fwk_id_t api_id,
    const void **api)
{
    *api = &tc1_system_system_power_driver_api;
    return FWK_SUCCESS;
}

static int tc1_system_start(fwk_id_t id)
{
    int status;
    unsigned int i;
...
static int tc1_system_process_notification(
...
const struct fwk_module module_tc1_system = {
    .type = FWK_MODULE_TYPE_DRIVER,
    .api_count = MOD_TC1_SYSTEM_API_COUNT,
    .init = tc1_system_mod_init,
    .bind = tc1_system_bind,
    .process_bind_request = tc1_system_process_bind_request,
    .process_notification = tc1_system_process_notification,
    .start = tc1_system_start,
};

const struct fwk_module_config config_tc1_system = { 0 };
