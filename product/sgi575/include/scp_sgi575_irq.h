...
#ifndef SCP_SGI575_IRQ_H
#define SCP_SGI575_IRQ_H

#include <fwk_interrupt.h>

#define SCP_WDOG_IRQ FWK_INTERRUPT_NMI /* SCP Watchdog (SP805) */

enum scp_sgi575_interrupt {
    ...
    PLL_CORES1_UNLOCK_IRQ      = 58, /* Consolidated PLL unlock for PLLs
                                        33-64, 97-128, 161-192, 225-256 */
    CLUSTER_FAULT_IRQ          = 59, /* Consolidated fault IRQ for all
                                        clusters */
    ...
    INT_FAULTNS_IRQ            = 100, /* Non-Sec fault interrupt from
                                         Interconnect PPU */
    INT_PMU_DTC0_IRQ           = 101, /* PMU count overflow interrupt if
                                         DTC > 0 */
    INT_PMU_DTC1_IRQ           = 102, /* PMU count overflow interrupt if
                                         DTC > 1 */
    INT_PMU_DTC2_IRQ           = 103, /* PMU count overflow interrupt if
                                         DTC > 2 */
    INT_PMU_DTC3_IRQ           = 104, /* PMU count overflow interrupt if
                                         DTC > 3 */
