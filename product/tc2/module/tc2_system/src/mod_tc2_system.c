...
static unsigned int scmi_notification_table[1] = {
    (unsigned int)SCP_TC2_SCMI_SERVICE_IDX_PSCI,
};

/* Module context */
struct tc2_system_ctx {
...
struct tc2_system_isr {
    unsigned int interrupt;
    void (*handler)(void);
};

static struct tc2_system_ctx mod_ctx;

static const uint32_t feature_flags =
    (TC2_SDS_FEATURE_FIRMWARE_MASK | TC2_SDS_FEATURE_DMC_MASK |
     TC2_SDS_FEATURE_MESSAGING_MASK);
...
static int tc2_system_shutdown(enum mod_pd_system_shutdown system_shutdown)
{
    NVIC_SystemReset();
...
static const struct mod_system_power_driver_api
    tc2_system_system_power_driver_api = {
        .system_shutdown = tc2_system_shutdown,
    };
...
static int tc2_system_mod_init(
    fwk_id_t module_id,
    unsigned int unused,
    const void *unused2)
{
...
static int tc2_system_bind(fwk_id_t id, unsigned int round)
{
    int status;
...
static int tc2_system_process_bind_request(
    fwk_id_t requester_id,
    fwk_id_t pd_id,
    fwk_id_t api_id,
    const void **api)
{
    *api = &tc2_system_system_power_driver_api;
    return FWK_SUCCESS;
}

static int tc2_system_start(fwk_id_t id)
{
    int status;
    unsigned int i;
...
static int tc2_system_process_notification(
...
const struct fwk_module module_tc2_system = {
    .type = FWK_MODULE_TYPE_DRIVER,
    .api_count = MOD_TC2_SYSTEM_API_COUNT,
    .init = tc2_system_mod_init,
    .bind = tc2_system_bind,
    .process_bind_request = tc2_system_process_bind_request,
    .process_notification = tc2_system_process_notification,
    .start = tc2_system_start,
};

const struct fwk_module_config config_tc2_system = { 0 };
