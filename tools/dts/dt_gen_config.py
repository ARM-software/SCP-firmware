...
# make the local version of the python devicetree module visible
...
"""
    ...
    ### not really used for SP but look to see what it does.
...
def relativize(path):
    ...
    try:
        return str("SCP_BASE_DIR" / pathlib.Path(path).relative_to(scp_base_dir))
    except ValueError:
...
def parse_input():
    ...
    parser.add_argument("--verbose", help="extra script logging for debug")
