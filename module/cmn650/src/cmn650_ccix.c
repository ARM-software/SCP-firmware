...
static void program_cxg_ha_id(struct cmn650_device_ctx *ctx, uint8_t haid)
{
    struct cmn650_cxg_ha_reg *cxg_ha_reg;
    ...
    const struct mod_cmn650_ccix_config *ccix_config)
{
    ...
    FWK_LOG_INFO(MOD_NAME "Exchanging protocol credits for link %d...", linkid);
    /* Exchange protocol credits using link up bit */
    cxg_ra_reg->LINK_REGS[linkid].CXG_PRTCL_LINK_CTRL |= CXG_LINK_CTRL_UP_MASK;
    cxg_ha_reg->LINK_REGS[linkid].CXG_PRTCL_LINK_CTRL |= CXG_LINK_CTRL_UP_MASK;
    FWK_LOG_INFO(
    ...
    const struct mod_cmn650_ccix_config *ccix_config)
{
    ...
    FWK_LOG_INFO(MOD_NAME "Entering system coherency for link %d...", linkid);
    ...
    const struct mod_cmn650_ccix_config *ccix_config)
{
    ...
    FWK_LOG_INFO(MOD_NAME "Entering DVM domain for link %d... Done", linkid);
