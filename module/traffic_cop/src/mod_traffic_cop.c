...
static int tcop_update(struct perf_plugins_perf_update *data)
{
    ...
    domain_idx = fwk_id_get_element_idx(data->domain_id);
...
static int tcop_report(struct perf_plugins_perf_report *data)
{
    ...
    domain_idx = fwk_id_get_element_idx(data->dep_dom_id);
    if (domain_idx >= tcop_ctx.tcop_domain_count)
        return FWK_SUCCESS;
    ...
    const void *data)
{
    ...
    domain_ctx = &tcop_ctx.domain_ctx[fwk_id_get_element_idx(domain_id)];
...
static int tcop_start(fwk_id_t id)
{
    ...
    domain_ctx = &tcop_ctx.domain_ctx[fwk_id_get_element_idx(id)];
