...
static void *_lookup(struct r_node *cur,
                    struct r_node *cand, uint32_t iid, int bitno)
{
    if (NULL == cur) {
...
static int _add(struct r_node **cur,
                uint32_t iid, void *entry, int bitsize, int bitno)
{
    struct r_node *new;
...
static void gicd_write_isenabler(
    uintptr_t base, unsigned int id, unsigned int val)
{
    unsigned int n = id >> ISENABLER_SHIFT;
...
static void gicd_set_ipriorityr(
    uintptr_t base, unsigned int id, unsigned int pri)
{
...
static int set_isr_dummy_param(
    void (*isr)(uintptr_t param), uintptr_t parameter)
{
    return FWK_SUCCESS;
}
